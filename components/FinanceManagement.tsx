...
import { 
  ...
  ArrowRight, FileText, Banknote, ShieldCheck
} from 'lucide-react';
...
const FinanceManagement: React.FC<FinanceManagementProps> = ({ orders, expenses, lang }) => {
  ...
  const summary = useMemo(() => {
    const cash = Math.round(completedOrders.filter(o => o.paymentMethod === 'Cash').reduce((s, o) => s + o.totalAmount, 0));
    const digital = Math.round(completedOrders.filter(o => o.paymentMethod !== 'Cash').reduce((s, o) => s + o.totalAmount, 0));
    return { cash, digital, total: cash + digital };
  }, [completedOrders]);

  const handleEndShift = () => {
    if (!confirm('确定结束当前班次并生成交班报告吗？')) return;
    ...
    setTimeout(() => {
      ...
      alert('本班次已结算。交班报告已生成并发送至管理员审计终端。');
    }, 2000);
  };
  ...
  return (
    <div className="space-y-8 pb-20 animate-fade-up">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
           <h2 className="text-3xl font-bold text-slate-900 tracking-tight">{t('finance')}</h2>
           <p className="text-sm text-slate-500 mt-1">营收流水审计与前台收银交班管理中心</p>
        </div>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm group hover:border-blue-500 transition-all">
                 ...
                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">今日累计总营收</p>
          ...
          <div className="bg-white rounded-[3.5rem] border border-slate-100 shadow-sm overflow-hidden">
            <div className="px-10 py-8 border-b border-slate-100 flex items-center justify-between">
               <h3 className="text-lg font-black uppercase tracking-tight">流水明细 / Daily Transaction Log</h3>
               ...
               <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100 font-bold text-slate-700">
                     {completedOrders.length === 0 ? (
                       <tr><td colSpan={4} className="px-10 py-20 text-center text-slate-300">暂无今日成交流水</td></tr>
                     ) : (
                       completedOrders.map(o => (
                         <tr key={o.id} className="hover:bg-slate-50 transition-colors">
                            <td className="px-10 py-6 text-xs text-slate-400">{new Date(o.createdAt).toLocaleTimeString()}</td>
                            <td className="px-10 py-6">#{o.roomId} - {o.id.slice(-6)}</td>
                            <td className="px-10 py-6 text-right font-mono text-slate-900">{C}{Math.round(o.totalAmount).toLocaleString()}</td>
                            <td className="px-10 py-6 text-center">
                              <span className="px-4 py-1 bg-blue-50 text-blue-600 rounded-lg text-[9px] font-black uppercase tracking-widest">
                                {o.paymentMethod}
                              </span>
                            </td>
                         </tr>
              ...
              <section className="bg-white p-12 rounded-[4rem] border border-slate-100 shadow-sm space-y-10 relative overflow-hidden">
                 ...
                 <div className="flex items-center space-x-6">
                    ...
                    <div>
                       ...
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">当前班次起始: {new Date().toLocaleDateString()} 09:00</p>
                    </div>
                 </div>
                 ...
                 <div className="grid grid-cols-2 gap-8">
                    <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 group hover:bg-white hover:border-blue-500 transition-all">
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">班次内现金汇总</p>
                    ...
                    <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 group hover:bg-white hover:border-indigo-500 transition-all">
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">班次内网银汇总</p>
                 ...
                 <div className="pt-10 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <button 
                      ...
                      className="px-12 py-5 bg-red-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl hover:bg-slate-950 transition-all active-scale flex items-center gap-3"
                    >
           ...
           <div className="lg:col-span-4 space-y-6">
              <div className="bg-slate-900 p-10 rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden">
                 ...
                 <div className="flex items-center gap-3 mb-8">
                    ...
                    <h4 className="text-sm font-black uppercase tracking-widest text-slate-400">交班历史审计</h4>
                 </div>
                 <div className="space-y-4 max-h-[400px] overflow-y-auto no-scrollbar">
                    {shiftHistory.length === 0 ? (
                      <div className="py-20 text-center text-slate-600 border border-dashed border-slate-700 rounded-3xl">
                        <History size={32} className="mx-auto mb-4 opacity-20" />
                        <p className="text-[10px] font-black uppercase tracking-widest">暂无历史交班数据</p>
                      </div>
                    ) : (
                      shiftHistory.map((shift, i) => (
                        <div key={i} className="p-5 bg-white/5 rounded-2xl border border-white/5 flex items-center justify-between group cursor-pointer hover:bg-white/10 transition-all">
                           <div className="flex items-center space-x-4">
                              ...
                              <div>
                                <p className="text-xs font-bold">班次 #{shift.id.slice(-6)}</p>
                                <p className="text-[8px] text-slate-500 uppercase tracking-widest">{new Date(shift.date).toLocaleString()}</p>
                              </div>
                           </div>
                           <div className="text-right">
                              <p className="text-xs font-black text-white">{C}{shift.total.toLocaleString()}</p>
                              <p className="text-[8px] text-emerald-500 uppercase font-black">Success</p>
                           </div>
                        </div>
                      ))
                    )}
