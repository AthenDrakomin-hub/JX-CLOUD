...
const ImageLibrary: React.FC<ImageLibraryProps> = ({ materials, onAddMaterial, onDeleteMaterial, lang }) => {
  ...
  const handleIngSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingIng) await api.ingredients.update(data);
               ...
               <div key={ing.id} className="bg-white p-8 rounded-[3.5rem] border border-slate-50 shadow-sm hover:shadow-2xl transition-all duration-700 group relative overflow-hidden animate-in fade-in slide-in-from-bottom-8" style={{ animationDelay: `${idx * 50}ms` }}>
                  ...
                  <div className="flex items-start justify-between mb-10">
                     ...
                     <div className="flex flex-col space-y-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button onClick={async () => { if(confirm('确定删除该物料？')) { await api.ingredients.delete(ing.id); fetchIngredients(); } }} className="p-3 bg-slate-50 text-slate-400 hover:text-red-500 hover:bg-white hover:shadow-md rounded-xl transition-all"><Trash2 size={16} /></button>
                     </div>
                  </div>
          ...
          {filteredMaterials.map((m, i) => (
            <div key={m.id} className="group flex flex-col space-y-4 animate-in fade-in slide-in-from-bottom-8" style={{ animationDelay: `${i * 50}ms` }}>
              <div className="relative aspect-square rounded-[3.5rem] overflow-hidden bg-slate-100 shadow-xl border border-white p-2">
                <img src={m.url} className="w-full h-full object-cover rounded-[3rem] transition-transform duration-[2s] group-hover:scale-110" alt={m.name} />
                <div className="absolute inset-0 bg-slate-950/60 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center space-x-3 p-6 backdrop-blur-[2px]">
                   <button onClick={() => { navigator.clipboard.writeText(m.url); setCopiedId(m.id); setTimeout(() => setCopiedId(null), 2000); }} className="p-4 bg-white text-slate-900 rounded-2xl hover:bg-[#d4af37] hover:text-white transition-all shadow-xl">
                     {copiedId === m.id ? <Check size={20} className="text-emerald-500" /> : <Copy size={20} />}
                   </button>
                   <button onClick={() => onDeleteMaterial(m.id)} className="p-4 bg-white text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all shadow-xl">
                     <Trash2 size={20} />
                   </button>
                </div>
              </div>
