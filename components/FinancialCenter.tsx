...
// Fix: Language is exported from translations.ts, not types.ts
import { Order, Expense, Partner } from '../types';
import { translations, Language } from '../translations';
import { 
  Wallet, Handshake, CreditCard, Sparkles, 
  ShieldCheck, ArrowRightLeft, Landmark
} from 'lucide-react';
...
const FinancialCenter: React.FC<FinancialCenterProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'revenue' | 'partners' | 'gateways'>('revenue');
  const t = (key: keyof typeof translations.zh) => (translations[lang] as any)[key] || key;

  const tabs = [
    { id: 'revenue', icon: Landmark, label: '资金流水' },
    { id: 'partners', icon: Handshake, label: '联营合伙' },
    { id: 'gateways', icon: CreditCard, label: '支付网关' }
  ] as const;
    ...
    <div className="space-y-8 animate-fade-up">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white p-8 rounded-[3.5rem] border border-slate-200 shadow-sm relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center space-x-6">
           ...
           <div>
              <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase">财务与清算中控台</h2>
