    ...
    try {
      ...
      if (!status.ok) {
        setDiagResult({ 
          status: 'error', 
          message: status.msg, 
          hint: '连接握手失败。通常是 VITE_SUPABASE_URL 或 ANON_KEY 配置错误。' 
        });
      } else {
        ...
        <div className="relative z-10 space-y-4">
          ...
          <div className="flex flex-wrap gap-4 pt-4">
             ...
             >
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">运行完整性诊断</span>
             </button>
          </div>
        </div>
            ...
            {errorInfo ? (
              <div className="p-20 text-center space-y-6 animate-in zoom-in-95">
                ...
                <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 text-left max-w-2xl mx-auto shadow-2xl relative">
                   ...
                   <h5 className="text-blue-400 text-[10px] font-black uppercase tracking-widest">Fix Protocol: SQL Manual Override</h5>
                   ...
                   <code className="text-[11px] font-mono text-emerald-400 block leading-relaxed bg-black/50 p-4 rounded-xl border border-white/5">
                     DROP POLICY IF EXISTS "Global Access" ON {selected};<br/>
                     CREATE POLICY "Global Access" ON {selected} FOR SELECT TO anon USING (true);<br/><br/>
                   </code>
                </div>
              </div>
            ) : (
