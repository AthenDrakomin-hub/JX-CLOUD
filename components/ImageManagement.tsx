...
const ImageManagement: React.FC<ImageManagementProps> = ({ lang }) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      alert('Format error: Please select an image file.');
      return;
    }
  ...
  return (
    <div className="space-y-10 animate-fade-up">
        ...
        <div className="lg:col-span-2 bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm relative overflow-hidden flex flex-col justify-center min-h-[180px]">
           ...
           <div className="relative z-10 space-y-2">
              <div className="flex items-center space-x-2 text-blue-600">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-[0.4em]">{t('cloudStorageGateway')}</span>
              </div>
              <h2 className="text-4xl font-black text-slate-900 tracking-tighter">{t('visualAssetCenter')}</h2>
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">
                {t('storageVault')}: <span className="text-blue-500">jiangxiyunchu</span> · {files.length} {lang === 'en' ? 'Objects' : '个对象'}
              </p>
           </div>
        </div>

        <div className="bg-slate-950 p-10 rounded-[3rem] shadow-2xl flex flex-col justify-center space-y-6">
           <div className="flex items-center justify-between">
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('opCommand')}</p>
              <button 
           ...
           <label className={`group flex items-center justify-center space-x-4 h-18 rounded-[1.5rem] font-black text-xs uppercase tracking-[0.2em] transition-all cursor-pointer shadow-xl active:scale-95 ${isUploading ? 'bg-slate-800 text-slate-500' : 'bg-blue-600 text-white hover:bg-white hover:text-slate-950'}`}>
              ...
              <span>{isUploading ? t('syncingNow') : t('uploadingNewAsset')}</span>
      ...
      {/* 检索过滤条 */}
      <div className="bg-white/60 backdrop-blur-xl p-4 rounded-[2rem] border border-white shadow-sm flex items-center px-8 sticky top-28 z-30">
        <Search className="text-slate-300 mr-4" size={20} />
        <input 
          value={searchTerm} 
          onChange={e => setSearchTerm(e.target.value)} 
          placeholder={lang === 'en' ? "Search filename..." : "输入文件名进行快速检索..."}
        ...
        <div className="hidden md:flex items-center space-x-2 ml-4 px-5 border-l border-slate-200">
           ...
           <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest whitespace-nowrap">{filteredFiles.length} {lang === 'en' ? 'Results' : '个结果'}</span>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="h-[400px] bg-white rounded-[4rem] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-slate-200 space-y-6">
           ...
           <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('galleryEmpty')}</p>
        </div>
      ) : (
          ...
          {filteredFiles.map((file, idx) => (
            ...
            >
               <div className="relative aspect-square overflow-hidden bg-slate-50 p-2">
                  ...
                  <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col items-center justify-center p-6 space-y-2 backdrop-blur-[2px]">
                     <div className="flex gap-2 w-full">
                        ...
                        >
                          ...
                          <span>{copiedKey === file.key ? 'Done' : (lang === 'en' ? 'Copy URL' : '复制链接')}</span>
                        </button>
                     </div>
                     <button 
                     ...
                     >
                       <Trash2 size={14} />
                       <span>{lang === 'en' ? 'Destroy Asset' : '彻底删除'}</span>
                     </button>
                  </div>
               </div>
      ...
      {previewImage && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 sm:p-20 bg-slate-950/95 backdrop-blur-2xl animate-in fade-in duration-500">
           ...
           <div className="relative w-full h-full flex flex-col items-center justify-center">
              <div className="max-w-4xl w-full aspect-square md:aspect-video rounded-[3rem] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.5)] border-4 border-white/10 bg-slate-900 relative group">
                 <img 
                    src={previewImage.url} 
                    alt="Preview" 
                    className="w-full h-full object-contain"
                 />
                 <div className="absolute bottom-10 left-10 right-10 p-8 bg-slate-950/80 backdrop-blur-xl rounded-[2rem] border border-white/10 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-all transform translate-y-4 group-hover:translate-y-0">
                    <div className="space-y-1">
                       <p className="text-white font-bold text-lg tracking-tight">{previewImage.key}</p>
                       <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">Dimension Info: Auto-Scaling Enabled</p>
                    </div>
                    <button 
                      onClick={() => copyToClipboard(previewImage.url, previewImage.key)}
                      className="px-8 py-4 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center space-x-2 hover:bg-blue-500 transition-all"
                    >
                       <Copy size={16} />
                       <span>{copiedKey === previewImage.key ? 'Copied URL' : (lang === 'en' ? 'Copy Resource Link' : '复制资产链接')}</span>
                    </button>
                 </div>
              </div>
           </div>
