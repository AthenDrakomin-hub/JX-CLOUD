...
/**
 * 清理数据库并重置三级分类结构（仅使用数据库中存在的字段）
 */
async function cleanupAndResetSimple() {
  ...
  try {
    ...
    if (level3 && level2 && level1) {
      if (level3.level === 3 && level2.level === 2 && level1.level === 1 &&
          level3.parentId === '102' && level2.parentId === '101' && level1.parentId === null) {
