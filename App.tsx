...
import { Bell, Command, Loader2, ShieldCheck, Wifi, WifiOff, Globe } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const savedLang = localStorage.getItem('jx_lang_preference');
    if (savedLang && ['en', 'zh', 'fil'].includes(savedLang)) {
      ...
      localStorage.setItem('jx_lang_preference', defaultLang);
    }
  }, [i18n]);
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('orders_realtime_v11').on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, (payload: any) => {
        if (payload.eventType === 'INSERT') {
          ...
          notificationService.broadcastOrderVoice(updatedOrder, i18n.language as Language);
      ...
      <div className={`min-h-screen transition-colors duration-500 ${config?.theme === 'dark' ? 'dark bg-slate-950' : 'bg-slate-50'}`} style={{ fontFamily: config?.fontFamily || 'Plus Jakarta Sans' }}>
        ...
        <main className={`transition-all duration-500 min-h-screen ${isSidebarCollapsed ? 'ml-24' : 'ml-72'}`}>
          ...
          <div className="p-10 lg:p-12 max-w-[1600px] mx-auto min-h-[calc(100vh-100px)]">
            {currentTab === 'dashboard' && <Dashboard orders={orders} rooms={rooms} expenses={expenses} dishes={dishes} ingredients={[]} partners={partners} currentUser={session.user as any} />}
            {currentTab === 'rooms' && <RoomGrid rooms={rooms} dishes={dishes} categories={categories} onUpdateRoom={async(r) => { await api.rooms.updateStatus(r.id, r.status); refreshData(); }} onRefresh={refreshData} />}
