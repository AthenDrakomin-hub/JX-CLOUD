...
const PaymentManagement: React.FC<PaymentManagementProps> = ({ lang }) => {
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const updated: PaymentMethodConfig = {
      ...
      qr_url: formData.get('qr_url') as string,
               ...
               <div className="space-y-6">
                  ...
                  <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 min-h-[80px]">
                    <p className="text-xs font-medium text-slate-500 leading-relaxed">
                      {lang === 'zh' ? p.description : p.description_en}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-6">
                     ...
                     <div className="shrink-0 p-3 bg-white rounded-2xl border-2 border-slate-100 shadow-sm relative group/qr">
                        {p.qr_url || p.wallet_address ? (
                          <QRCodeSVG value={p.qr_url || p.wallet_address || ''} size={80} level="M" />
                        ) : (
                          <div className="w-20 h-20 bg-slate-100 rounded-lg flex items-center justify-center text-slate-200">
                             <ImageIcon size={24} />
                          </div>
                        )}
                        <div className="absolute inset-0 bg-slate-900/10 opacity-0 group-hover/qr:opacity-100 transition-opacity rounded-2xl flex items-center justify-center">
                     ...
                     <div className="flex-1 space-y-3 min-w-0">
                        ...
                        <div className="relative group/addr">
                           <div className="w-full bg-white border border-slate-200 p-4 rounded-xl text-[10px] font-mono text-slate-600 truncate pr-10 shadow-inner">
                              {p.wallet_address || '未配置接入点'}
                           </div>
                           {p.wallet_address && (
                             <button 
                               onClick={() => handleCopy(p.wallet_address!, p.id)}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                   <div className="space-y-8">
                      <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">排序权重 (Order)</label><input name="sort_order" type="number" defaultValue={editingPayment.sort_order} required className="w-full px-8 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-bold text-sm" /></div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">English Name</label><input name="name_en" defaultValue={editingPayment.name_en} required className="w-full px-8 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-bold text-sm" /></div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-6">
                        ...
                        <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">符号</label><input name="currency_symbol" defaultValue={editingPayment.currency_symbol} required className="w-full px-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-serif text-lg text-center" /></div>
                        <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">PHP 汇率</label><input name="exchange_rate" type="number" step="0.0001" defaultValue={editingPayment.exchange_rate} required className="w-full px-4 py-4 bg-blue-50 border border-blue-100 rounded-2xl outline-none font-black text-blue-700 text-center text-sm" /></div>
                      </div>
                   ...
                   <div className="space-y-8 bg-slate-50 p-8 rounded-[3rem] border border-slate-200 relative overflow-hidden">
                      ...
                      <div className="space-y-6">
                         <div className="space-y-3">
                            ...
                            <input name="wallet_address" defaultValue={editingPayment.wallet_address} disabled={isFieldLocked} className={`w-full px-6 py-4 rounded-2xl outline-none font-mono text-xs transition-all border ${isFieldLocked ? 'bg-slate-100 border-slate-100 text-slate-400' : 'bg-white border-blue-500 ring-4 ring-blue-50 text-slate-900 shadow-inner'}`} />
                         </div>
                         <div className="space-y-3">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">外部跳转链接 (QR Payload)</label>
                            <input name="qr_url" defaultValue={editingPayment.qr_url} disabled={isFieldLocked} className={`w-full px-6 py-4 rounded-2xl outline-none font-mono text-xs transition-all border ${isFieldLocked ? 'bg-slate-100 border-slate-100 text-slate-400' : 'bg-white border-blue-500 ring-4 ring-blue-50 text-slate-900 shadow-inner'}`} />
                         </div>
                         <div className="flex flex-col items-center justify-center p-8 bg-white rounded-[2.5rem] border border-slate-100 shadow-inner">
                            {editingPayment.qr_url || editingPayment.wallet_address ? (
                              <div className="p-4 bg-white rounded-2xl border-2 border-slate-50 shadow-sm">
                                <QRCodeSVG value={editingPayment.qr_url || editingPayment.wallet_address || ''} size={140} level="H" />
                              </div>
                            ) : (
