 ...
 * 获取响应式组件尺寸
 */
export const getResponsiveComponentSize = (size: 'sm' | 'md' | 'lg' | 'xl' = 'md'): { padding: string; fontSize: string; borderRadius: string } => {
  const scale = getResponsiveFontScale();
  
  const sizes = {
    sm: { padding: `${8 * scale}px ${16 * scale}px`, fontSize: `${12 * scale}px`, borderRadius: `${6 * scale}px` },
    md: { padding: `${12 * scale}px ${24 * scale}px`, fontSize: `${14 * scale}px`, borderRadius: `${8 * scale}px` },
    lg: { padding: `${16 * scale}px ${32 * scale}px`, fontSize: `${16 * scale}px`, borderRadius: `${12 * scale}px` },
    xl: { padding: `${20 * scale}px ${40 * scale}px`, fontSize: `${18 * scale}px`, borderRadius: `${16 * scale}px` }
  };
