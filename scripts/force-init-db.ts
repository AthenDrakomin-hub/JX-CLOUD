...
import { user as authUser, users, systemConfig, rooms } from '../drizzle/schema.js';
...
async function initializeUsers() {
  ...
  try {
    ...
    if (existingRootUser.length === 0) {
      ...
      await db.update(users).set({ role: 'admin' }).where(eq(users.email, 'athendrakomin@proton.me'));
      console.log('Root admin user role updated to admin');
    }
...
// Force run initialization regardless of environment checks
console.log('Force running database initialization...');
