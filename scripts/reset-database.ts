// scripts/reset-database.ts - 重置数据库脚本
...
async function resetDatabase() {
  try {
    ...
    console.log('📊 user 表记录数:', userCountResult.rows[0].count);
    
    const usersCountResult = await db.execute(sql`SELECT COUNT(*) FROM "users"`);
    console.log('📊 users 表记录数:', usersCountResult.rows[0].count);
    
    const passkeysCountResult = await db.execute(sql`SELECT COUNT(*) FROM "passkeys"`);
    console.log('📊 passkeys 表记录数:', passkeysCountResult.rows[0].count);
