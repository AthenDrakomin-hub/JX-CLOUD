// scripts/check-tables-fixed.ts - 检查数据库表结构（修复版）
...
async function checkTables() {
  try {
    ...
    for (const row: any of tables.rows) {
      try {
        // 使用模板字符串的正确语法
        const result = await db.execute(sql.raw(`SELECT COUNT(*) as count FROM "${row.tablename}";`));
        console.log(`  ${row.tablename}: ${result.rows[0].count} 条记录`);
      } catch (e) {
        console.log(`  ${row.tablename}: 无法查询（可能是视图或特殊表）- ${(e as Error).message}`);
      }
    }
