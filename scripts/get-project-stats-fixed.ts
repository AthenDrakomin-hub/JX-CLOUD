// scripts/get-project-stats-fixed.ts - 获取项目统计信息（修复版本）
  ...
  try {
    ...
    if (partnerCount > 0) {
      ...
      try {
        ...
        allPartners.forEach((partner, index) => {
          console.log(`  ${index + 1}. ${partner.name} (${partner.id}) - 状态: ${partner.status || 'N/A'}, 负责人: ${partner.ownerName || 'N/A'}`);
        });
      } catch (error) {
        console.log('  无法获取详细合作伙伴信息，可能由于数据库表结构差异');
      }
    }
    ...
    if (adminsWithPasskeys > 0) {
      ...
      for (const admin of adminUsers) {
        ...
        console.log(`  • ${admin.name} (${admin.email}) - ${passkeyCount > 0 ? `绑定 ${passkeyCount} 个生物识别凭证` : '未绑定生物识别'}`);
      }
    }
